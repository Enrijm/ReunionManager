...
public class CustomAuthentificationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain, Authentication authentication) throws IOException, ServletException {
        User user = (User)authentication.getPrincipal(); // this is the user from Spring Security and we got it from the authentication
        Algorithm algorithm = Algorithm.HMAC256("secret".getBytes()); // this algorithm is for signed the JWT and JWRT
        ...
        String refresh_token = JWT.create()
                .withSubject(user.getUsername())
                .withExpiresAt(new Date(System.currentTimeMillis() + 40*60*1000 )) // 40 min will expired the token
                .withIssuer(request.getRequestURL().toString())
                .sign(algorithm);
